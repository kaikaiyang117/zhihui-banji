...
- 更新源：客户端优先读取 `https://home.kaikaiyang.top/updates/update-manifest.json` 并从自建服务器下载；检查或下载失败时回退到 GitHub Releases API / GitHub Release 清单。自建源用版本化目录保留旧安装包，通过 `scripts/publish-update-server.sh` 在服务端校验 SHA-256 后最后原子替换根清单；安装包在客户端仍必须通过 SHA-256 校验。修改更新接口或校验流程时，同步更新 `server/tests/integration/mig09.test.ts` 和 `docs/发布检查清单.md`。
...
`.github/workflows/ci.yml` 会执行后端测试、前端构建和浏览器 UI 冒烟测试；`.github/workflows/windows-installer.yml` 会在 `main` 推送或手动触发时构建 Windows x64 安装包、运行安装冒烟测试并保存 Actions Artifact；版本标签发布由 `release.yml` 负责，其构建 Windows x64、macOS arm64 和 macOS x86_64 三个安装包。发布前还应参考 `docs/发布检查清单.md`，但不要把发布操作当作普通代码修改的一部分自动执行。
