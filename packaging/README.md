...
程序会打开本机浏览器；进入工作台后点击右上角“手机访问”，手机或平板连接同一 Wi-Fi 后扫描 5 分钟有效的单次配对二维码即可访问：

```bash
MeimeiWorkbench --lan
```

`--lan` 仍保留用于手动排查。默认端口为 5000，若被其他程序占用，程序会自动选择后续可用端口。二维码只能使用一次；配对设备可在电脑端撤权，请仅授权可信设备。
...
局域网模式启动后由电脑端按需生成短时配对码，配对成功后设备获得可撤销凭证。数据库只保存凭证哈希，服务重启后授权仍有效。该模式仍只适合可信网络，不要把端口映射到公网。
