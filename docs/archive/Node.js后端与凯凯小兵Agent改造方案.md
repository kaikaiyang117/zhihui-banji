...
> 归档说明：迁移工作已完成，本文件仅用于追溯历史决策，不作为当前实现、开发顺序或发布依据。当前资料以仓库根目录的 `AGENTS.md` 和 `docs/` 下现行文档为准。
...
3. [`docs/系统功能开发计划.md`](../系统功能开发计划.md)：业务功能依赖和业务闭环定义。
4. [`docs/Agent能力矩阵.md`](../Agent能力矩阵.md)：Agent 工具、参数、风险和渠道权限的当前登记。
5. [`docs/Agent能力矩阵.md`](../Agent能力矩阵.md)、[`docs/Agent回归报告.md`](../Agent回归报告.md)和[`docs/Agent与微信接入配置.md`](../Agent与微信接入配置.md)：Agent、评测和微信现状。
...
## 20.10 MIG-09 交付记录（2026-08-11）

- 服务（3 个子代理并行移植，A/B 对拍验证）：`reports.ts`（四类报告 + 档案快照/导出）、`health.ts`（目标/提醒/复盘/汇总 + 多 sheet 导出）、`exportService.ts` 扩展（7 类 Excel 导出，openpyxl 可解析）、`migrationService.ts`（迁移包导出/恢复，路径穿越拒绝 + 完整性校验，node:zlib 手写 zip 与 Python zipfile 字节级等价）、`update.ts`（检查/下载/校验状态机 + ready_to_install + installer-path + token）。
- `db/index.ts`：数据库单例 + `restoreFromBuffer`（校验→备份→替换→重开）。
- 路由 `mig09.ts` 约 30 条；`/api/reports/ai/preview` 暂 503（AGENT-00 接入）。
...
## 20.12 AGENT-01 交付记录（2026-08-11）

- `src/agent/graph/`（state/graph）+ `runner.ts`：LangGraph StateGraph 复刻 Python runner 语义（直接工具路由、规则/模型计划、条件与引用、空批量恢复一次、失败重建一次、重试熔断、计划最终流式回答）；SqliteSaver 检查点（`agent-checkpoints.db`，thread_id=session_id）；自定义流事件 plan/plan_step/delta。
- `src/agent/planner.ts`（子代理）：规则计划 + 模型计划（JSON/DSML 解析、验证）。
- 路由 `agent.ts`：状态/配置/工具/chat/chat-stream/会话/审计/用量（配置仅本机）。
- 验证（9 项新增，总 162/162）：直接路由、计划执行、全班不退化、熔断、流式事件、检查点落库无思维链、模型未配置 Agent 报错业务正常、HTTP。
- 关键实现：节点名不能与状态属性同名（`plan` 冲突 → create_plan/execute_plan_steps）；LangGraph 自定义事件经 `config.writer`；`ToolCall` 为扁平结构。

## 20.13 AGENT-02 交付记录（2026-08-11）

- `src/agent/actions.ts`：确认写入状态机（预览/确认/取消/过期/幂等/备份/失败保留）、`handleConfirmation` 聊天拦截、`pendingForSession`；四个写工具执行路径均写 `agent_action` 来源键。
- 接线：runner.chat/chatStream 首轮确认拦截；`/api/agent/actions/pending|confirm|cancel` 路由；tools 路由传 session_id；会话压缩已生效（compaction 摘要标记）。
...
## 20.15 MIG-10 交付记录（2026-08-11）

- Electron 升级并精确锁定 `43.3.0`（上一代 33 已停止维护；macOS 本地验证 ABI 148 / Node 24.18.1）。
- `desktop/main.js`：后端改走 Node 入口（`--desktop-child --lan`，桌面默认开启局域网配对以支撑“手机访问”）；打包模式 `utilityProcess.fork(resources/server/dist/entry.js)`，开发模式系统 Node 子进程（`server/node_modules` 为系统 Node ABI）；异常退出最多自动重启 2 次，之后弹窗选择重启/退出；退出仍由 Electron 终止后端并等待端口释放。
- 打包：`scripts/build-node-bundle.{sh,ps1}` 组装 `build/server-bundle/`（dist + static + app-version.json + 生产依赖），better-sqlite3 通过 `@electron/rebuild` 重建为打包 Electron ABI；`electron-builder.yml` 的 `extraResources` 指向 server-bundle，不再打包 Python sidecar；版本唯一来源 `APP_VERSION` 同步写入 app-version.json、桌面 package.json 与安装包。
- 补缺：`/api/stats/dashboard|calendar|attendance|scores|points|fund` 路由（MIG-05 声称“统计”但前端依赖缺失，Dashboard 首页 404）；新增 `server/src/services/stats.ts`、`server/src/http/routes/stats.ts`、`tests/integration/stats.test.ts`（5 项）。
