...
微信 Bot Token、模型 API Key 和微信同步 Token 会保存到数据目录下独立的权限受限 JSON 文件（文件权限为当前用户可读写），不会写入业务 SQLite 或迁移包。生产环境仍建议优先使用环境变量注入凭据。
...
批量学生查询只允许字段白名单，当前支持学号、姓名、性别、出生年月、民族、监护人姓名/关系/职业、第二监护人姓名/关系/职业、是否住校、特长和班级任职，不返回监护人电话、家庭住址或备注。
...
两个命令效果相同，只会清空当前微信用户与“班小助”的 Agent 对话记录，不会删除智汇·班记业务数据。
